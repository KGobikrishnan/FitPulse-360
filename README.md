...
[![GitHub Stars](https://img.shields.io/github/stars/KGobikrishnan/FitPulse-360?style=for-the-badge&logo=star&color=F59E0B&logoColor=white)](https://github.com/KGobikrishnan/FitPulse-360)
[![React 19](https://img.shields.io/badge/React%2019-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)](https://react.dev/)
[![Spring Boot 3.3](https://img.shields.io/badge/Spring_Boot_3.3-6DB33F?style=for-the-badge&logo=spring-boot&logoColor=white)](https://spring.io/projects/spring-boot)
[![PostgreSQL 16](https://img.shields.io/badge/PostgreSQL_16-316192?style=for-the-badge&logo=postgresql&logoColor=white)](https://www.postgresql.org/)
...
[![Docker Multi-Stage](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)](https://www.docker.com/)
...
| :--- | :--- | :--- | :--- |
| <img src="https://img.shields.io/badge/Role-ADMIN-4F46E5?style=flat-square"/> | `admin@fitlife.com` | `admin123` | Three.js 3D Studio, Financial P&L, GST Tax Invoice PDF Generator, 1-Click CSV/Excel Exporter, 24-Locker grid, Mini POS, Turnstile Telemetry |
| <img src="https://img.shields.io/badge/Role-TRAINER-9333EA?style=flat-square"/> | `trainer@fitlife.com` | `trainer123` | Dynamic Client Roster, Interactive Workout Routine Protocol Builder, Macro Diet Chart Composer, Week-by-Week Progress Review Vault |
| <img src="https://img.shields.io/badge/Role-MEMBER-10B981?style=flat-square"/> | `user@fitlife.com` | `user123` | Live Workout Set Logger, 60s Rest Countdown Stopwatch, 10-Glass Water Tracker, PostgreSQL PR Vault (1-Rep Max Confetti), Gate Pass |
...
```mermaid
flowchart TD
    subgraph ClientLayer["🖥️ Frontend Tier (React 19 + PWA v2.5 + Three.js)"]
        UI["macOS Liquid Glass UI & Aurora Mesh"]
        Three["Three.js 3D WebGL Canvas"]
        SW["Service Worker v2.5 (Network-First & Offline Sync)"]
        CameraQR["Live Web Camera Kiosk Scanner (10 FPS)"]
        PDF["GST Tax Invoice Print Engine"]
    end

    subgraph GatewayLayer["🌐 Edge Proxy & Security Tier"]
        Nginx["Nginx Reverse Proxy (:80 / :3000)"]
        JWT["Spring Security 6 (Stateless JWT + RBAC)"]
    end

    subgraph BackendLayer["☕ Spring Boot 3.3.3 Enterprise Core (JDK 21)"]
        AdminCtrl["AdminController (P&L, POS, Invoicing)"]
        TrainerCtrl["TrainerController (Protocols, Diets)"]
        MemberCtrl["MemberController (PR Vault, Passes)"]
        AuthCtrl["AuthController (JWT Rotation)"]
    end

    subgraph DataLayer["💾 Multi-Model Persistence Tier"]
        Postgres[("🐘 PostgreSQL 16 (Relational Schema)")]
        Redis[("⚡ Redis 7 (Token Cache & Rate Limits)")]
    end
    ...
    JWT --> BackendLayer
    AdminCtrl --> Postgres
    TrainerCtrl --> Postgres
    MemberCtrl --> Postgres
    AuthCtrl --> Redis
    SW -.->|Offline Queue Flush| MemberCtrl
```
